...
const API_BASE_URL = buildApiUrl(API_CONFIG.ENDPOINTS.BARBER_SHOPS);
...
export const useBarberShopSearch = () => {
      ...
      try {
        ...
        if (displaySalons.length === 0) {
          showNotification({
            type: "info",
            title: "No Results",
            message: "No barber shops match your search criteria",
            duration: 3000,
          });
        }
      } catch (error) {
  ...
  const initializeData = useCallback(async () => {
    ...
    try {
      ...
      showNotification({
        type: "success",
        title: "Barber Shops Loaded",
        message: `Found ${displaySalons.length} barber shop(s)`,
        duration: 3000,
      });
