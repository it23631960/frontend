...
const API_BASE_URL = buildApiUrl(API_CONFIG.ENDPOINTS.BRIDAL_SALONS);

export const useBridalSalonSearch = () => {
      ...
      try {
        ...
        if (displaySalons.length === 0) {
          showNotification({
            type: "info",
            title: "No Results",
            message: "No bridal salons match your search criteria",
            duration: 3000,
          });
        }
      } catch (error) {
        const errorMessage =
          error instanceof ApiServiceError
            ? error.message
            : "Failed to search bridal salons";
  ...
  const initializeData = useCallback(async () => {
    ...
    try {
      ...
      showNotification({
        type: "success",
        title: "Bridal Services Loaded",
        message: `Found ${displaySalons.length} bridal service(s)`,
        duration: 3000,
      });
